...
class VGG16(object):
    ...
    def build(self, rgb, train_mode=None):
        ...
        self.logits = get_fc_layer(self.relu7, self.fc_feat_size, self.num_classes, 'fc8')
    ...
    def fc_layer_from_scratch(self, bottom, in_size, out_size, name):
        with tf.variable_scope(name):
            weights, biases = self.get_fc_var_from_scratch(in_size, out_size, name)
    ...
    def get_fc_var_from_scratch(self, in_size, out_size, name):
        initial_value = tf.truncated_normal([in_size, out_size], 0.0, 0.001)
        weights = self.get_var_from_scratch(initial_value, name, 0, name + '_weights')

        initial_value = tf.truncated_normal([out_size], .0, .001)
        biases = self.get_var_from_scratch(initial_value, name, 1, name + '_biases')

        return weights, biases

    def get_var(self, initial_value, name, idx, var_name):
        if self.data_dict is not None and name in self.data_dict:
            print('Load weight {}/{}'.format(name, var_name))
    ...
    def get_var_from_scratch(self, initial_value, name, idx, var_name):
        value = initial_value
        if self.trainable:
            var = tf.Variable(value, name=var_name)
        else:
            var = tf.constant_initializer(value, dtype=tf.float32, name=var_name)
        self.var_dict[(name, idx)] =  var
        return var
